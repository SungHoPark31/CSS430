...
public class SysLib {
    ...
    // files to be created, (i.e., the number of inodes to be allocated)
    // in the file system. The return value is 0 on success, otherwise -1.
    ...
    // mode (where "r" = ready only, "w" = write only,
    // "w+" = read/write, "a" = append), and allocates a new file 
    // descriptor, fd to this file. The file is created if it
    // does not exist in the mode "w", "w+" or "a".
    public static int open(String fileName, String mode) {
        String[] inputArgs = {fileName, mode};
        return Kernel.interrupt(Kernel.INTERRUPT_SOFTWARE, Kernel.OPEN, 0, inputArgs);
    }

    // reads up to buffer.length bytes from the file indicated by
    // fd, starting at the position currently pointed to by the seek pointer.
    // The return value is the number of bytes that have
    ...
    // writes the contents of buffer to the file indicated by
    // fd, starting at the position indicated by the seek pointer.
    // The return value is the number of bytes that have
    // been written, or a negative value upon an error
    ...
    // not destroyed until the last open on it is closed, but new attempts to open it will fail.
