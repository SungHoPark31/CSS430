...
{   @SuppressWarnings({"unchecked","rawtypes"})
	...
	while ( true ) {
		...
		synchronized(queue) {		
		    ...
		    //Check if you've visited all slice[level]
		    //If level == 0, slice[level] == 1, go rotate back to 0
		    //and add current TCB to next level in the queue. 
		    if((level == 0) && (slice[level] == 1)) {
			slice[level] = 0;			
			queue[0].remove(currentTCB);
			queue[1].add(currentTCB);
      		    }
		    //If level == 1, slice[level] == 2, go rotate back to 0       
                    //and add current TCB to next level in the queue.    
		    ...
		    //If level == 2, slice[level] == 4, go rotate back to 0       
                    //and add current TCB to the end of the queue at level 2. 
