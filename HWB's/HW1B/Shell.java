	    ...
	    // now args[] got "cmd1 arg ... ;or& cmd2 arg ... ;or& ...
            ...
            for ( int i = 0; i < args.length; i++ ) {
                if ( args[i].equals( ";" ) || args[i].equals( "&" )
                    ...
                    if ( command != null ) {
                        // HW1B:
                        //Check if command[0] is "exit". If so, Terminate.
                        if(command[0].compareTo("exit") == 0) {
                            SysLib.exit();
                            return;
                        }
                        else {
                            //Otherwise, pass command to SysLib.exec()
                            int exec = SysLib.exec(command);

                            //Keep calling SysLib.join() as long as args[i] is not
                            //"&". Also, make sure to repeat SysLib.join() until the
                            //return value is exactly the same as the return value
                            //for SysLib.exec()
                            while((!args[i].equals("&")) && (SysLib.join() != exec));
                        }
                    }
                    first = i + 1;
                }
